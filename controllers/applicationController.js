...
router.get(
  '/customerList',
  [verifyToken, checkPermissions.check('read', 'Customer')],
...
router.post(
  '/customerAdd',
  [verifyToken, checkPermissions.check('create', 'Customer')],
  function (req, res) {
    const {
      first_name,
      last_name,
...
router.put(
  '/customerEdit/:id',
  [verifyToken, checkPermissions.check('update', 'Customer')],
  function (req, res) {
    const {
      first_name,
      last_name,
...
router.delete(
  '/customerDelete/:id',
  [verifyToken, checkPermissions.check('delete', 'Customer')],
  function (req, res) {
    Customer.findByIdAndRemove(req.params.id, function (err, data) {
      ...
      fs.unlink(
        path.join(
          __dirname,
          '../public/applicationImages/' + data.profileImage
        ),
        function (err) {
    ...
    Customer.findByIdAndUpdate(
      ...
      function (err, Customer) {
        ...
        fs.unlink(
          path.join(
            __dirname,
            '../public/applicationImages/' + req.body.oldImage
          ),
          function (err) {
            if (err && err.code == 'ENOENT') {
              // file doens't exist
...
router.get(
  '/faqList',
  [verifyToken, checkPermissions.check('read', 'Faq')],
  function (req, res, next) {
    let page = parseInt(req.query.currentPage, 10) || 1;
    let limit = parseInt(req.query.limit, 10) || 10;
    let fsort = req.query.sort;
...
router.post(
  '/faqAdd',
  [verifyToken, checkPermissions.check('create', 'Faq')],
  function (req, res) {
...
router.put(
  '/faqEdit/:id',
  [verifyToken, checkPermissions.check('update', 'Faq')],
  function (req, res) {
    const { title, content, status } = req.body;
...
router.delete(
  '/faqDelete/:id',
  [verifyToken, checkPermissions.check('delete', 'Faq')],
  function (req, res) {
...
router.get(
  '/serviceList',
  [verifyToken, checkPermissions.check('read', 'Service')],
  function (req, res, next) {
    let page = parseInt(req.query.currentPage, 10) || 1;
    let limit = parseInt(req.query.limit, 10) || 10;
    let fsort = req.query.sort;
...
router.post(
  '/serviceAdd',
  [verifyToken, checkPermissions.check('create', 'Service')],
  function (req, res) {
    const { title, content, status } = req.body;
...
router.put(
  '/serviceEdit/:id',
  [verifyToken, checkPermissions.check('update', 'Service')],
  function (req, res) {
    const { title, content, status } = req.body;
...
router.delete(
  '/serviceDelete/:id',
  [verifyToken, checkPermissions.check('delete', 'Service')],
  function (req, res) {
    Service.findByIdAndRemove(req.params.id, function (err, data) {
      ...
      fs.unlink(
        path.join(
          __dirname,
          '../public/applicationImages/' + data.serviceImage
        ),
        function (err) {
          if (err && err.code == 'ENOENT') {
            // file doens't exist
...
router.get(
  '/roleList',
  [verifyToken, checkPermissions.check('read', 'Role')],
  function (req, res, next) {
    let page = parseInt(req.query.currentPage, 10) || 1;
    let limit = parseInt(req.query.limit, 10) || 10;
    let fsort = req.query.sort;
...
router.post(
  '/roleAdd',
  [verifyToken, checkPermissions.check('create', 'Role')],
  function (req, res) {
...
router.put(
  '/roleEdit/:id',
  [verifyToken, checkPermissions.check('update', 'Role')],
  function (req, res) {
    const { name, description, permissions } = req.body;
...
router.delete(
  '/roleDelete/:id',
  [verifyToken, checkPermissions.check('delete', 'Role')],
  function (req, res) {
...
router.get(
  '/userList',
  [verifyToken, checkPermissions.check('read', 'Users')],
  function (req, res, next) {
    let page = parseInt(req.query.currentPage, 10) || 1;
    let limit = parseInt(req.query.limit, 10) || 10;
    let fsort = req.query.sort;
...
router.post(
  '/userAdd',
  [verifyToken, checkPermissions.check('create', 'Users')],
  function (req, res) {
...
router.put(
  '/userEdit/:id',
  [verifyToken, checkPermissions.check('update', 'Users')],
  function (req, res) {
...
router.delete(
  '/userDelete/:id',
  [verifyToken, checkPermissions.check('delete', 'Users')],
  function (req, res) {
...
router.get('/refreshUserPermission', [verifyToken], function (req, res) {
  ...
  User.findOne({ _id: req.userId }).exec(function (err, user) {
    Role.findOne({ _id: user.role_id }, function (err, role) {
      if (err)
        return res.status(httpStatus.OK).send(`Server error: ${err.message}`);
...
router.get(
  '/getDashboard',
  [verifyToken, checkPermissions.check('read', 'Dashboard')],
  function (req, res, next) {
    //counts
    Promise.all([
      ...
      User.aggregate([
        ...
        return res.status(httpStatus.OK).send({
          status: true,
          data: collections,
          totalProduct: counts[0],
          totalCustomer: counts[1],
          totalBlog: counts[2],
          totalUser: counts[3],
        });
      });
  ...
  function (req, res) {
    ...
    User.findByIdAndUpdate(
      ...
      res.status(httpStatus.OK).send({
        status: true,
        msg: 'Data Updated Successfully',
        userData: { name: name, role_id: role_id, email: user.email },
      });
    });
  }
);
  ...
  function (req, res) {
    ...
    User.findOne({ _id: req.params.id }, function (error, user) {
      ...
      if (passwordMatch) {
        ...
        res
          .status(httpStatus.OK)
          .send({ status: false, msg: 'Current Password Does Not Match' });
      }
    });
  }
);
